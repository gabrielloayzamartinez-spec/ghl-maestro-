...
export async function auditAdAttribution(contactId, options = {}) {
  ...
  try {
    ...
    for (const conv of conversations) {
      ...
      while (nextUrl) {
        ...
        for (const m of messages) {
          // Diferenciar Meta Ads de Social Media (Orgánico)
          const fbMeta = m.meta?.fb || {};
          
          // Verificar si el contacto tiene etiquetas de pauta
          const tags = contact.tags || [];
          const hasAdTags = tags.some(t => t.includes('ad_id') || t.includes('messenger_ads') || t.includes('meta-ads'));
          
          // Es un clic de pauta SOLO si tiene un Ad ID explícito o si el contacto vino por pauta y es un mensaje de Facebook
          const isFromAd = Boolean(
            m.direction === 'inbound' && (
              fbMeta.adId || 
              m.meta?.email?.adId || 
              (hasAdTags && (m.source === 'facebook' || fbMeta.pageName))
            )
          );
    ...
    // REGLA DE NEGOCIO: La duplicidad (X2, X3) SOLO se cuenta si proviene de la MISMA OFICINA (Página Meta).
    // Si el clic histórico pertenece a otra página, no suma al multiplicador actual.
    const clicksForSameOffice = uniqueAdClicks.filter(click => click.pageName === latestInteraction.pageName);

    // Contabilizar clics únicos solo para la sede actual
    const totalAdClicks = Math.max(clicksForSameOffice.length, 1);
